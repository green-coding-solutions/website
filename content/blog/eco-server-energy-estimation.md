...
Something we often encounter when talking to clients is that they often have no idea of how much energy the various components in their pipeline use. We have developed multiple projects like [Eco-CI]({{< relref path="products/eco-ci" >}}) that measures the energy usage of CI/CD pipelines or [Power Hog]({{< relref path="products/power-hog" >}}) which logs energy consumption on desktops. But to date is was not possible to see how much your servers use.

In our effort to open source most of our developments we are happy to announce the public release of the [Eco-Server](https://github.com/green-coding-solutions/eco-server-energy-estimation) project and the release of the CarbonDB backend in the [Green Metrics Tool]({{< relref path="products/green-metrics-tool" >}}). This is a big step for us a company as it enables us to monitor even more resources in the Software Life Cycle. To read more about Software Life Cycle Assessment please check out our articles [SLCA PART 1]({{< relref path="blog/software-life-cycle-assessment" >}}) and [SLCA PART 2]({{< relref path="blog/carbon-aware-development" >}}).
...
See an example whow the data looks on our [Green Metrics Tool Dashboard](https://metrics.green-coding.io/carbondb.html), which is also included in Carbon DB.
